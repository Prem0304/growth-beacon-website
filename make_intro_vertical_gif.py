...
# Config
WIDTH, HEIGHT = 540, 960 # 9:16 aspect ratio (portrait) for mobile
SCALE = 2 # Render at 1080x1920, scale down to 540x960
...
def main():
    output_path = "growth_beacon_intro_vertical.gif"
    print("Generating vertical frames...")
    
    # Branding coordinates on Render canvas (1080x1920)
    X_CENTER = W_RENDER // 2
    Y_BASE = 1000 # Placed in the upper-middle of vertical space
    ...
    for frame_idx in range(TOTAL_FRAMES):
        ...
        # 1. SPARKLE FIRST (Fades in from frame 0 to 25)
        ...
        if cur_r > 1:
            ...
            glow_color_layer = Image.new("RGB", (W_RENDER, H_RENDER), (242, 201, 76)) # Gold
        ...
        # 2. RISING GRAPH (Starts at frame 30 sequentially)
        if frame_idx >= 30:
        ...
        # 3. TYPOGRAPHY (Starts fading in at frame 75, positioned below the logo Y=1220)
        if frame_idx >= 75:
            ...
            # Larger text size on portrait
            title_font = get_font(bold=True, size=52)
        ...
        # Resize to vertical GIF size (540x960)
    ...
    print("Saving animated vertical GIF...")
    frames_list[0].save(
        output_path,
        save_all=True,
    ...
    print(f"Vertical GIF saved successfully at {output_path}!")
