...
SCALE = 2 # Render at 4K, scale down to 1080p for ultra-smooth anti-aliasing
...
def main():
    output_path = "growth_beacon_intro.mp4"
    print("Initializing video writer...")
    
    # Setup OpenCV Video Writer (avc1 is H.264, native for browser playback)
    # Falling back to mp4v if avc1 fails locally (standard OpenCV builds can be picky)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video = cv2.VideoWriter(output_path, fourcc, FPS, (WIDTH, HEIGHT))
    
    # Branding coordinates on Render canvas (3840x2160)
    X_CENTER = W_RENDER // 2
    Y_BASE = 1560
    ...
    for frame_idx in range(TOTAL_FRAMES):
        ...
        # 1. BEACON LIGHT FIRST (Fades in from frame 0 to 30, active throughout)
        ...
        if cur_r > 1:
            ...
            beam_color_layer = Image.new("RGB", (W_RENDER, H_RENDER), (242, 201, 76)) # Gold
        ...
        if frame_idx >= 35:
            # Bar 1 (Left): starts rising at frame 35, ends at 75
            pct1 = min(1.0, max(0.0, (frame_idx - 35) / 40))
            cur_h1 = H1 * ease_in_out_quad(pct1)
            
            # Bar 2 (Middle): starts rising at frame 47, ends at 87
            pct2 = min(1.0, max(0.0, (frame_idx - 47) / 40))
            cur_h2 = H2 * ease_in_out_quad(pct2)
            
            # Bar 3 (Right): starts rising at frame 59, ends at 99
        ...
        if frame_idx >= 90:
            ...
            title_font = get_font(bold=True, size=116)
            sub_font = get_font(bold=False, size=36)
            
            # Draw "GROWTH BEACON" (with split colors)
